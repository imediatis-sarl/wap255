 ...
 */
    ...
    if (!empty($search)) {
        ...
        $conditions['filename'] = "%{$search}%";
        $conditions['description'] = "%{$search}%";
    }

    # Récupérer le nombre total de vues pour ce module
    $totalRecords = View::_getTotalForModule($moduleId);

    # Récupérer le nombre de vues filtrées pour ce module
    $totalRecordWithFilter = View::_getTotalWithFilter($conditions);

    # Construire la clause d'ordre
    $orderBy = "{$columns[$orderColumn]} {$orderDir}";

    # Récupérer les données
    $data = View::_getRecords($conditions, $orderBy, $start, $length);
